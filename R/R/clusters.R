...
errors_scores <- function(df, balance = rep(1, 3), ts_validation = TRUE, ...) {
  ...
  scores <- df %>%
    ...
    mutate(
      decomp.rssd_w = balance[1] * .data$decomp.rssd_n,
      MAPE_train_w = balance[2] * .data$MAPE_train_n,
      mape_w = balance[3] * .data$mape_n
    ) %>%
    # Calculate error score
    mutate(error_score = sqrt(.data$decomp.rssd_w^2 + .data$MAPE_train_w^2 + .data$mape_w^2)) %>%
...
.prepare_df <- function(x, all_media, dep_var_type, cluster_by) {
  if (cluster_by == "performance") {
    ...
    errors <- distinct(
      x, .data$solID, .data$decomp.rssd, .data$MAPE_train, .data$mape
    )
    ...
    if (cluster_by == "hyperparameters") {
      outcome <- select(
        ...
        contains(c("decomp.rssd", "MAPE_train", "mape"))
      ) %>%
        removenacols(all = FALSE)
    }
  }
...
.plot_topsols_errors <- function(df, top_sols, limit = 1, balance = rep(1, 3)) {
  ...
  left_join(df, select(top_sols, 1:3), "solID") %>%
    ...
    ggplot(aes(x = .data$decomp.rssd, y = .data$MAPE_train)) +
    ...
    labs(
      ...
      x = "DECOMP.RSSD", y = "MAPE_train",
      caption = sprintf(
        "Weights: DECOMP.RSSD %s%%, MAPE_train %s%%, mape %s%%",
...
.plot_topsols_rois <- function(df, top_sols, all_media, limit = 1) {
  real_rois <- as.data.frame(df)[, -c(which(colnames(df) %in% c("mape", "decomp.rssd", "MAPE_train")))]
